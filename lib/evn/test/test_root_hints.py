...
class TestKnownRootAncestorRenamedWithRootExclusion(EnversionTest,
                                                    unittest.TestCase):
    def test_01(self):
        repo = self.create_repo()
        svn = repo.svn
        ...
        dot()
        svn.cp(repo.ra('/other/'), repo.ra('/other.bak/'), m='IGNORE ERRORS')

        evnadmin.add_root_exclusion(
            repo.name,
            root_exclusion='/ignore/',
        )
        svn.cp(repo.ra('/other/'), repo.ra('/ignore/'), m='IGNORE ERRORS')
